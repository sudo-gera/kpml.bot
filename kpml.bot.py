...
if 1:
 ...
 for q in look():
  ...
  if q[1] == 'json':
   ...
   if ts:
    send(q[0],'теперь автоматические оповещения будут приходить вам в '+ts+'. Следует обратить внимание, что оповещение содержит только те изменения, которые опубликованы на момент этого оповещения')
   else:
