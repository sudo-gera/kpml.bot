...
#создаёт ссылку на автора. Формат входа: [id,text,platform] где text не используется и может быть чем угодно
def getlink(q):
 if q[2]=='vk':
  return getlink_vk(q[0])
 else:
  return 'пользователь с id '+str(q[0])+' с платформы '+str(q[2])

def getlink_vk(q):
 return 'vk.com/id'+str(q)
#dates########################################################
...
#теперь можно приступать к пониманию, чего хотел пользователь
  if q[1] == '':
   ...
   tmp=tmp
   send(tmp)
  elif q[1] == 'отключить пустые сообщения' or q[1] == 'пусто' and db[q[2]][q[0]]['dis_empty']==0:
   db[q[2]][q[0]]['dis_empty']=1
  ...
  elif q[1] == 'включить пустые сообщения' or q[1] == 'пусто' and db[q[2]][q[0]]['dis_empty']==1:
   db[q[2]][q[0]]['dis_empty']=0
