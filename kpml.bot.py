...
# сообщене получено
    shuffle(wai)
    # сообщения возвращаются в одном и том же порядке. Если есть сообщение, обработка которого завершилась неудачно, то в следующий раз это сообщение будет первым и до остальных дело снова не дойдёт. Поэтому их лучше перемешать случайным образом
    for q in wai:
...
# теперь можно приступать к пониманию, чего хотел пользователь
        ...
        #команды отладки, их использоание только сообщает какую-то информацию и ничего не делает
        elif text == 'отмена':
            send('отменено')
