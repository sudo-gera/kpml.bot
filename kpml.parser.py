...
#dates########################################################
# формат даты, который испоьзуется вовсей пограмме: день (число, как есть)
# месяц (число, нумеруются с 0, то есть от 0 до 11) год (число, как есть)
# день недели (не обязательно, число, нумеруются с 0, то есть от 0 до 6)
...
def parse():
    # предотвращение повторного открытия страницы, если она была открыта не
    # давно
    try:
    ...
    if time() - float(bt) > 300:
        try:
            q = urlopen(
                'http://kpml.ru/pages/raspisanie/izmeneniya-v-raspisanii').read().decode()
            if q != oq:
                log('site changed')
    ...
    q = [w for w in q if w and (
        w[0] != '\x02' or w[0] == '\x02' and w[-1] == '/')]
...
def repa(day, mon):
    ...
    for w in q:
        ...
        w[0] = '\0'.join(w[0])
        w[0] = w[0].split('\0')
        w[0] = [w[0][e] if e == 0 or w[0][e - 1] != '-' or w[0]
                [e] != '-' else '' for e in range(len(w[0]))]
        w[0] = [e for e in w[0] if e]
        for e in range(1, len(w[0]) - 1):
            if w[0][e] == '-':
                w[0][e] = '\0'.join(
                    list(map(str, range(int(w[0][e - 1]), int(w[0][e + 1])))))
        if w[0][0] == '-':
            w[0] = w[0][1:]
        if w[0][-1] == '-':
            w[0] = w[0][:-1]
...
# вспомогательная функция для проверки наличия данного класса среди
# нескольких, если фрагмент ориентирован на несоклько классов
...
def view(day=None, mon=None, _profile=None):
    if _profile is None and day is not None and mon is not None:
        global profile
    else:
        profile = _profile
    try:
        if day is None and mon is None:
            parsed = out()
        else:
            parsed = ''
...
def work(profile, empty=0):
    ...
    td = view(q, w, profile)
    if td or empty == 0:
        td = 'Изменения на сегодня, ' + \
            str(q) + ' ' + rmo[int(w)] + ' ' + rdw[dw] + ':\n' + td
    r, t, y, dw = next(q, w, e, dw)
    tn = view(r, t, profile)
    if tn or empty == 0:
        tn = 'Изменения на завтра, ' + \
            str(r) + ' ' + rmo[int(t)] + ' ' + rdw[dw] + ':\n' + tn
    if int(time()) % (24 * 3600) < 12 * \
            3600 or int(time()) % (24 * 3600) > 21 * 3600:
